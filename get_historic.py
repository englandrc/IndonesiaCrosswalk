# -*- coding: utf-8 -*-
"""
Created on Tue Jun 22 20:35:37 2021
...
def main():
    ...
    for i in range(0,len(parselist)):
        ...
        address = 'https://sig.bps.go.id/rest-historis/getwilayah?level=' + level + '&parent=' + item
        ...
        if i == 0:
            df.to_csv (root+'historic_'+level+'.csv', index = None)
        else:
            df.to_csv(root+'historic_'+level+'.csv', mode='a', header=False, index=None)
